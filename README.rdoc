...
Create a maildir in /home/user/mail

  require "maildir"
  maildir = Maildir.new("/home/user/mail") # creates tmp, new, and cur dirs
  # To skip directory creation, call Maildir.new("/home/user/mail", false)
...
List "cur" messages based on flags.
  ...
  maildir.list("cur", :flags => 'ST') # => lists all messages with flag 'S' and 'T'; Flags must be specified in acending ASCII order ('ST' and not 'TS')
...
* user docelic (github[http://github.com/docelic])

== Copyright

Copyright (c) 2017 user docelic. See LICENSE for details.
